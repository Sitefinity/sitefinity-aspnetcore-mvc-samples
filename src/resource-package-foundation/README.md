...
## Npm
### Install
Npm is the package manager for JavaScript that enables you to assemble packages of reusable code. For more information, visit [Node.js](https://nodejs.org/) and [npm](https://www.npmjs.com/) websites. Currently *Foundation* package supports Node.js LTS version 12.17.0.
```
> npm install
> npm start
```
`npm start` executes the default **npm** scripts and subsequently watches for any changes in the files after.

## Package structure ##

The Foundation package contains foundation front-end assets, widget template and grid widget templates. Following is a hierarchical list of the major folders and files that come with the *Foundation* package.
...
By default, all widget templates are included in the Foundation package. To modify a widget template, simply open the respective template, say, *Horizontal* navigation template. To do so, navigate to `/ResourcePackages/Foundation/MVC/Views/Navigation`, open the `NavigationView.Horizontal.cshtml` file, and make your changes.
...
In this folder, you organize the project widget templates, located in either `/Mvc/Views/` or `/ResourcePackages/Foundation/MVC/Views`.
...
To modify a grid widget, for example, a grid widget with two columns, simply navigate to `/ResourcePackages/Foundation/GridSystem/Templates`, open `two-columns.html` and make your changes.
...
**Example:** To create a simple placeholder with `<section>` tag, create `section.html` in `/ResourcePackages/Foundation/GridSystem/Templates/`
```
